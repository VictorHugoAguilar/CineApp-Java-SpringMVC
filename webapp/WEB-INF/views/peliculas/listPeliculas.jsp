	...
	<div class="container theme-showcase" role="main">
		...
		<div class="table-responsive">
			<table class="table table-hover table-striped table-bordered">
				...
				<c:choose>
					<c:when test="${peliculas.content.size() < 1}">
						<p class="alert alert-danger">No hay películas aun</p>
					</c:when>
					<c:otherwise>
							...
							<tr>
								...
								<td><c:choose>
										<c:when test="${ pelicula.estatus eq 'Activa' }">
											<span class="label label-success">${ pelicula.estatus }
											</span>
										</c:when>
										<c:otherwise>
											<span class="label label-danger">${ pelicula.estatus }
											</span>
										</c:otherwise>
									</c:choose></td>
								<td><a href="${ urlEdit }/${ pelicula.id }"
									...
									onclick='return confirm("¿Estas Seguro?")' title="Eliminar"><span
